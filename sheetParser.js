...
// ──────────────────────────────────────────────────────────
// 메인 진입점 (GitHub Actions: node sheetParser.js 로 직접 실행)
// ──────────────────────────────────────────────────────────

async function parseAndExportSheets() {
    // GitHub Secrets에서 서비스 계정 키를 JSON 문자열로 받음
    const serviceAccountKey = process.env.GOOGLE_SERVICE_ACCOUNT_KEY;
    if (!serviceAccountKey) {
        throw new Error('환경변수 GOOGLE_SERVICE_ACCOUNT_KEY 가 없습니다.');
    }

    const auth = new google.auth.GoogleAuth({
        credentials: JSON.parse(serviceAccountKey),   // 파일이 아닌 JSON 객체로 직접 전달
    ...
    if (!spreadsheetId) {
        throw new Error('환경변수 GOOGLE_SPREADSHEET_ID 가 없습니다.');
    }

    // 시트 목록 조회 — 이름이 _ 로 시작하는 시트 제외
